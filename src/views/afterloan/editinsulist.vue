        ...
        >
          <div class="top">
            <div class="formtitle">
              ...
              <!-- <span class="time">{{data.cdate}}</span> -->
            </div>
            ...
            >
              ...
              <el-col :span="24">
                ...
                <div v-if="isoriginal == '1'">
                  <el-col :span="12">
                    <el-form-item label="收件人" class="smallsize" :label-width="formLabelWidth">
                      <el-input v-model="form.recperson" size="small"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="12">
                    <el-form-item class="smallsize" label="收件人电话" :label-width="formLabelWidth">
                      <el-input
                  ...
                  <el-col :span="24">
                    <el-form-item class="smallsize" label="收件地址" :label-width="formLabelWidth">
          ...
          <div class="botm">
            ...
            <viewer :images="imgList">
              <div class="insulistImglist">
                ...
                >
                  <img v-if="img.url.indexOf('http') == -1" :src="imageUrl+img.url">
...
<script>
import { submitinsu, saveinsu, editinsu, getlist, save } from "@/api/afterloan";
import { getToken } from "@/utils/auth";
import { isEmpty, objectReplace, objectOnly } from "@/utils/index";
export default {
  data() {
    return {
      isLoading: true,
  ...
  methods: {
    ...
    submit(index) {
      let para = objectReplace(this.form, { status: "type" });
      if (para.inmd5 != "" && para.inmd5 != null) {
        para.inmd5 = para.inmd5.join(",");
      } else {
        para.inmd5 = "";
      }
    ...
    datail() {
      ...
      editinsu(para)
        .then(res => {
          ...
          if (res.data.success) {
            this.data = res.data.data;
            this.isoriginal = this.data.isoriginal;
            if (res.data.data.reportno == null) {
              res.data.data.reportno = "";
            }
            var form = objectOnly(res.data.data, [
              "memo",
              "reportno",
              "recaddress",
              "reportdate",
            ...
            if (res.data.data.reportdate != null) {
              form.reportdate = res.data.data.reportdate;
            } else {
              form.reportdate = "";
            }
            this.form = objectReplace(form, { type: "status" });
            if (res.data.data.inmd5 != "" && res.data.data.inmd5 != null) {
              var md5arry = res.data.data.inmd5.split(",");
              this.form.inmd5 = md5arry;
              for (let i = 0; i < md5arry.length; i++) {
                var md5 = {
                  url: md5arry[i],
                  status: false
                };
                this.imgList.push(md5);
              }
            } else {
              this.form.inmd5 = [];
            }
...
.addInsulist /deep/ .liuyan .el-badge__content {
  line-height: 16px;
}
