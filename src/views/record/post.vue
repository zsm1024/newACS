  ...
  <section>
    <el-row>
      <el-col :span="11">
        <el-breadcrumb separator="/" class="app-breadcrumb">
          ...
          <el-breadcrumb-item>抵押资料管理</el-breadcrumb-item>
        </el-breadcrumb>
      </el-col>
      <el-col :span="13" align="right" class="topmiddle" style="padding-right:0">
    ...
    >
      <el-form-item class="blocktime">
        <strong class="demonstration">提交时间</strong>
        <el-date-picker
    ...
    <el-row class="custom-card-grid">
    <div class="tipstext">
      <p>资料库存不足的情况下请提前2个工作日进行申请；收到申请后5个工作日内安排寄出并反馈单号</p>
    </div>
    <el-table
      :data="lists"
    ...
    >
      ...
      <el-table-column align="center" width="80" label="联系人" prop="LINKMAN" show-overflow-tooltip></el-table-column>
      <el-table-column align="center" width="150" label="手机号" prop="PHONE"></el-table-column>
      <el-table-column align="center" width="150" label="邮寄地址" prop="MAILLINGADDRESS"></el-table-column>
...
export default {
  ...
  methods: {
      ...
      postLIst(para).then(res => {
        ...
        if (res.data.success) {
          ...
          for (let i = 0; i < this.lists.length; i++) {
            this.lists[i].CDATE = this.lists[i].CDATE.split(" ")[0];
          }
        } else {
          this.$message({
            type: "error",
    ...
    add_date() {
      this.$router.push({
        path: "/dealer/record/addPost/" + 0 + "/" + 0
      });
    },
    //查看
    view(id) {
      this.$router.push({
        path: "/dealer/record/addPost/" + 1 + "/" + id
      });
    ...
    addFile(id) {
      this.$router.push({
        path: "/dealer/record/addPost/" + 0 + "/" + id
      });
      // this.dialogView=true
    },
    //删除
    del(id) {
      ...
      this.$confirm("确定删除吗？", "提示", {
        ...
        .then(() => {
          delMortgageApply(para).then(res => {
            console.log("del", res);
            if (res.data.success) {
              this.$message({
                type: "success",
    ...
    determine() {
      ...
      if (this.huifu_cont == "") {
        ...
        para.RECORDID = "";
        para.SOMORTGAGEMATERIALID = this.msgid;
